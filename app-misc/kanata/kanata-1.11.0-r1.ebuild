...
src_install() {
	...
	newinitd "${FILESDIR}/kanata.initd" kanata

	insinto /etc/kanata
	doins "${FILESDIR}/kanata.kbd"
}

pkg_postinst() {
	elog "kanata runs as an OpenRC daemon under the kanata:kanata system user,"
	elog "starting before greetd so keyboard remaps are active at the greeter."
	elog ""
	elog "The default keymap at /etc/kanata/kanata.kbd is craig's Apple-M1"
	elog "home-row-mods + hyperkey + code/arrow layer config. Edit to taste."
	elog ""
	elog "To enable and start the service:"
	elog "    rc-update add kanata default"
	elog "    rc-service kanata start"
}
